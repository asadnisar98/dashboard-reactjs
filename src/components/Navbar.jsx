...
const NavButton = ({ title, customFunc, icon, color, dotColor }) => (
  <TooltipComponent style={{ display: 'inline-block' }} content={title} position="BottomCenter">
    <button
      ...
      <TooltipComponent content="Profile" position="BottomCenter">
        <div className="flex items-center gap-2 cursor-pointer p-1 hover:bg-light-gray rounded-lg" onClick={() => handleClick('userProfile')}>
            ...
            <p className="no-underline">
              <span className="text-gray-400 text-14 no-underline">Hi, </span> {' '}
              <span className="text-gray-400 font-bold ml-1 text-14 no-underline">Michael</span>
            </p>
            <MdKeyboardArrowDown className="text-gray-400 text-14 no-underline" />
        </div>
      </TooltipComponent>
